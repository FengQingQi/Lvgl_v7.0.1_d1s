...
static bool led_status2 = false;//LED1标志位
static bool led_status3 = false;//LED1标志位
...
static void lv_smarthome_boot_animation(int time)
{
    ...
    lv_bar_set_anim_time(bar, time*100);                            //设置lv_bar进度条的动画时长
    ...
    lv_label_set_recolor(label, true);                              //创建标签对象
...
static void lv_demo_tabview(void)
{
    tabview = lv_tabview_create(lv_scr_act(), NULL);                        //创建一个tabview，但未加载到显示
...
static void tabview2_create(lv_obj_t * parent)
{
    ...
    lv_imgbtn_toggle(btn_img2);                                     //设置翻转
	lv_obj_align(btn_img2, NULL, LV_ALIGN_IN_TOP_LEFT, 180, 50);    //左上角对齐
    lv_obj_set_event_cb(btn_img2, btn_event_led2);                  //设置按钮回调
    ...
    lv_imgbtn_toggle(btn_img3);                                     //设置翻转
	lv_obj_align(btn_img3, NULL, LV_ALIGN_IN_TOP_LEFT, 310, 50);    //左上角对齐
    lv_obj_set_event_cb(btn_img3, btn_event_led3);                  //设置按钮回调
...
static void watch_time(lv_obj_t * parent)
{
	...
	lv_obj_clean_style_list(hour_bg, LV_OBJ_PART_MAIN);             //清空对象风格
    ...
    lv_task_create((lv_task_cb_t)get_sys_time, 1000, LV_TASK_PRIO_LOW, NULL);     //1秒任务
}
...
static void get_sys_time(lv_obj_t * parent)
{
    ...
    //watch time
    lv_img_set_angle(hour_bg, Hour*60*5);                           //时钟旋转
    lv_img_set_angle(minute_bg, Minute*60);                         //分钟旋转
    lv_img_set_angle(second_bg, Second*60);                         //秒钟旋转
}
